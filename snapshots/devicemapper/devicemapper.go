...
func NewSnapshotter(root string) (snapshots.Snapshotter, error) {
	...
	if err := os.Mkdir(filepath.Join(root, "mounts"), 0700); err != nil && !os.IsExist(err) {
		return nil, err
	}
...
func (o *snapshotter) Remove(ctx context.Context, key string) (err error) {
	...
	for _, mountDir := range []string{path, o.getMountDir(id)} {
		err = o.lm.Unmount(mountDir)
		if err != nil {
			return errors.Wrap(err, "failed to unmount")
		}
	}

	err = os.Remove(o.getMountDir(id))
	if err != nil {
		return errors.Wrapf(err, "failed to remove mount dir: %s", volume)
	}
...
func (o *snapshotter) createSnapshot(ctx context.Context, kind snapshots.Kind, key, parent string, opts []snapshots.Opt) ([]mount.Mount, error) {
	...
	mountDir := o.getMountDir(s.ID)
	if err = os.MkdirAll(mountDir, 0700); err != nil {
		return nil, errors.Wrapf(err, "failed create path")
	}

	err = o.lm.Mount(s.ID, mountDir)
	if err != nil {
		return nil, errors.Wrapf(err, "failed to mount volume")
	}

	err = o.lm.BindRootfs(o.getMountDir(s.ID), path)
	if err != nil {
		return nil, errors.Wrapf(err, "failed to bind rootfs")
	}
...
func (o *snapshotter) getMountDir(id string) string {
	return filepath.Join(o.root, "mounts", id)
}
